  ...
  <div class="container-fluid">
    <div class="card">
      <div class="card-header card-header-primary">
        <h4 class="card-title">Isi kuesioner pengetahuan hipertensi</h4>
      </div>

      <div class="card-body col-sm-10">
      <iframe src="{{ asset('material') }}/img/silence.mp3" type="audio/mp3" allow="autoplay" id="audio" style="display: none"></iframe>
        <audio id="player" autoplay>
          <source src="{{ asset('material') }}/img/pengetahuan.mp3" type="audio/mp3">
      </audio>
        @if (session('status'))
        <div class="row">
          <div class="col-sm-12">
      ...
      @endif
        <div class="table-responsive">
          <table class="table table-hover">
            ...
            <tbody>
                <form method="post" action="{{ route('isi_pengetahuanSave') }}" autocomplete="off" class="form-horizontal">
                    @csrf
                    @method('post')
            <?php $no=1; ?>
            @foreach ($pengetahuan as $pengetahuan)
                <tr>
                    <td>{{ $no++ }}</td>
                    <td>{{ $pengetahuan->pertanyaan }}</td>
                    <td class="text-left">
                        <span><input type="radio" id={{ $pengetahuan->id }} name={{ $pengetahuan->id }} value="1" required/> Benar</span><br>
                        <span><input type="radio" id={{ $pengetahuan->id }} name={{ $pengetahuan->id }} value="0"/> Salah</span><br/>
                       <span><input type="radio" id={{ $pengetahuan->id }} name={{ $pengetahuan->id }} value="0"/> Tidak tahu</span>
                   </div>
                    </td>
                </tr>
            @endforeach
