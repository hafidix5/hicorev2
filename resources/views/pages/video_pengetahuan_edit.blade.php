...
@section('content')
  <div class="content">
    <div class="container-fluid">
      <div class="row">
        <div class="col-md-12">
          <form method="post" action="{{ route('video_pengetahuan.update',$video_pengetahuan->id ) }}" autocomplete="off" class="form-horizontal">
            @csrf
            @method('PUT')

            <div class="card ">
              <div class="card-header card-header-primary">
                <h4 class="card-title">{{ __('Ubah Video Pengetahuan') }}</h4>
              </div>
              <div class="card-body ">
                @if (session('status'))
                  <div class="row">
                ...
                <div class="row">
                  ...
                  <div class="col-sm-7">
                    <div class="form-group{{ $errors->has('nama') ? ' has-danger' : '' }}">
                      <input class="form-control{{ $errors->has('nama') ? ' is-invalid' : '' }}" name="nama" id="input-nama" type="text" placeholder="{{ __('') }}" value="{{$video_pengetahuan->nama}}" required="true" aria-required="true"/>
                      @if ($errors->has('nama'))
                ...
                <div class="row">
                  ...
                  <div class="col-sm-7">
                    <div class="form-group{{ $errors->has('video') ? ' has-danger' : '' }}">
                      <input class="form-control{{ $errors->has('video') ? ' is-invalid' : '' }}" name="video" id="input-video" type="text" placeholder="{{ __('Contoh : https://www.youtube.com/watch?v=vtt_kJ4kLFw') }}" value="{{$video_pengetahuan->video}}" required="true" aria-required="true"/>
                      @if ($errors->has('video'))
