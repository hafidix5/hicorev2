  ...
  <div class="container-fluid">
    <div class="card">
      <div class="card-header card-header-primary">
        <h4 class="card-title">Isi kuesioner Persepsi pasien hipertensi </h4>
      </div>
      ...
      @endif
        <div class="table-responsive">
          <table class="table table-hover">
            ...
            <tbody>
                <form method="post" action="{{ route('isi_persepsiSave') }}" autocomplete="off" class="form-horizontal">
                    @csrf
                    @method('post')
            <?php $no=1; ?>
            @foreach ($persepsi as $persepsi)
                <tr>
                    <td>{{ $no++ }}</td>
